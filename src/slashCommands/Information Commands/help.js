    ...
    const collector = m.createMessageComponentCollector({
      filter: (b) => {
        if (b.user.id === interaction.member.user.id) return true;
          ...
          b.reply({
            ephemeral: true,
            content: `Only **${interaction.member.user.tag}** can use this menu, run the command again.`,
          });
          return false;
        }
      },
    ...
    collector.on("collect", async (i) => {
      ...
      if (i.customId === "helpMenu") {
        ...
        if (value[0] === "home") {
          ...
          _commands = client.slashCommands
          ...
          editEmbed
            ...
            .setFooter({
              text: `© SXM_ABEL | Total: ${_commands.length} Configuration Commands`,
            });
          if (!m) return;
          return await interaction.editReply({
            embeds: [editEmbed],
